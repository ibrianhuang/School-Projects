...
public:
    ...
    void remove(const std::string& key);
   	...
   	// Deallocates and deletes the memory used by the member function buckets. Does NOT delete buckets itself.
	void deleteBuckets();
...
private:
    ...
    // The dynamically allocated array of pointers to linked lists. The "buckets"
    Node** buckets;

    // Keep track of the number of key/value pairs stored in the HashMap
    unsigned int mapSize;

    // Keeps track of the number of buckets currently dynamically allocated in the HashMap
    unsigned int mapBucketCount;


private:
	// Copies the contents of the parameter HashMap hm into the HashMap in which this function was called upom
	void copyBuckets(const HashMap& hm);

	// Decides which bucket the the hashed value refers to
	unsigned int bucketIndex(const unsigned int& hash, const unsigned int& bucketCount) const;
